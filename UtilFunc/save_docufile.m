function ret = save_docufile(data, folder)
...
if isempty(i) 
 %&& length(lookuptable) + 1 == data.uid
    save(sprintf('%s%s%s',folder,filesep, data.filename), '-struct', 'data');
    return;
end

if data.uid == i && strcmp(sprintf('%s.mat', data.filename), char(lookuptable{i})) == 1
    save(sprintf('%s%s%s%s%s',DOCUDIR, filesep,folder,filesep,data.filename), '-struct', 'data');
else
